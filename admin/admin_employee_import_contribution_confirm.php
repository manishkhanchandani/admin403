...
if($_POST['MM_Insert']==1) {
	if($_POST['contribution_date']) {
		...
		if($_FILES) {
			...
			if($array['detailIds']) {
				mysql_query("insert into employee_contribution_transaction set transaction_date = '".addslashes(stripslashes(trim($_POST['contribution_date'])))."', employer_id = 0") or die('error');
					...
					if($num>0) {
						...
						if($num2>0) {
							...
							if($plan['plan_id']>0) {	
								...
								$dob = $encryption->processDecrypt('dob', $employeeDetails['dob']);
...
if($totalRows_rsNoPlanId) {
	...
	do { 
		$tmpEmployee .= $row_rsNoPlanId['email'].' (SSN: '.$encryption->processDecrypt('ssn', $row_rsNoPlanId['ssn']).')<br>';
...
<form id="form2" name="form2" method="post" action="admin_employee_import_contribution_confirm2.php">
  <table width="100%" border="0" cellspacing="0" cellpadding="5">
    ...
    <tr>
      ...
      <td valign="top"><input name="contribution_date[<?php echo $key; ?>]" type="text" id="contribution_date_<?php echo $key; ?>" size="12" value="<?php echo $value['contribution_date']; ?>" /> <input type="button" value="Pick date" /></td>
