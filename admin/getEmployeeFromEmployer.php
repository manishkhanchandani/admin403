...
$query_rsEmployerPlan = sprintf("SELECT employee.employee_id, employee.email, employee.firstname, employee.ssn, employee.employer_id, employee.lastname FROM employee WHERE employee.employer_id = %s", $colname_rsEmployerPlan);
...
if($totalRows_rsEmployerPlan) {
	do { 
		...
		$xml .= '<name>' . htmlspecialchars($row_rsEmployerPlan['lastname'].', '.$row_rsEmployerPlan['firstname'].' / '.$row_rsEmployerPlan['ssn']) . '</name>'; 
		$xml .= '</message>'; 
