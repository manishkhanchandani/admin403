...
$return1 = getDelEmployersVendors($_GET['employer_id']);
if($return1) {
	$string1 = implode(', ', $return1);
} else {
	$string1 = 0;
}
$return2 = getEmployersVendors($_GET['employer_id']);
if($return2) {
...
$colname_rsVendor = "0";
if (isset($string3)) {
  $colname_rsVendor = (get_magic_quotes_gpc()) ? $string3 : addslashes($string3);
}
mysql_select_db($database_dw_conn, $dw_conn);
$query_rsVendor = sprintf("SELECT vendor_plan.plan_name, vendor.name, vendor.vendor_id, vendor_plan.plan_link, vendor_plan.plan_code, vendor_plan.plan_desc, vendor_plan.plan_id FROM vendor INNER JOIN vendor_plan ON vendor.vendor_id = vendor_plan.vendor_id WHERE vendor_plan.plan_id NOT IN (%s)", $colname_rsVendor);
$query_limit_rsVendor = sprintf("%s LIMIT %d, %d", $query_rsVendor, $startRow_rsVendor, $maxRows_rsVendor);
$rsVendor = mysql_query($query_limit_rsVendor, $dw_conn) or die(mysql_error());
...
if (isset($_GET['employer_id'])) {
  $colemployer_rsVendorDeleted = (get_magic_quotes_gpc()) ? $_GET['employer_id'] : addslashes($_GET['employer_id']);
}
...
if (isset($_GET['employer_id'])) {
  $colemployer_rsVendorActive = (get_magic_quotes_gpc()) ? $_GET['employer_id'] : addslashes($_GET['employer_id']);
}
...
$queryString_rsVendor = "";
if (!empty($_SERVER['QUERY_STRING'])) {
  $params = explode("&", $_SERVER['QUERY_STRING']);
  $newParams = array();
  foreach ($params as $param) {
    if (stristr($param, "pageNum_rsVendor") == false && 
        stristr($param, "totalRows_rsVendor") == false) {
      array_push($newParams, $param);
    }
  }
  if (count($newParams) != 0) {
    $queryString_rsVendor = "&" . htmlentities(implode("&", $newParams));
  }
}
$queryString_rsVendor = sprintf("&totalRows_rsVendor=%d%s", $totalRows_rsVendor, $queryString_rsVendor);
...
<br />
<br />
<table width="100%" border="6" cellspacing="0" cellpadding="3" class="browntbl">
    <tr valign="bottom" >
        <td colspan="2" class="brownth"><font color="#ffffff" face="Tahoma" size="2">Add Vendor <?php echo DISPLAYPLANNAME; ?>s</font></td>
    </tr>
    <tr valign="top" >
      <td colspan="2" class="browntd">
        ...
        <table width="100%" border="6" cellspacing="0" cellpadding="5" style="border-style:solid" bordercolor="#999999" class="tbl">
          <tr>
            <td valign="top" class="thcview6"><strong>Vendor</strong></td>
            <td valign="top" class="thcview6"><strong><?php echo DISPLAYPLANNAME; ?></strong></td>
            <td valign="top" class="thcview6"><strong><?php echo DISPLAYPLANNAME; ?> Code </strong></td>
            <td valign="top" class="thcview6"><strong><?php echo DISPLAYPLANNAME; ?> Description</strong></td>
            <td valign="top" class="thcview6"><strong>Actions</strong></td>
          </tr>
          <?php do { ?>
          <tr>
            <td valign="top" class="tdcview2"><?php echo $row_rsVendor['name']; ?></td>
            <td valign="top" class="tdcview2"><a href="<?php echo $row_rsVendor['plan_link']; ?>" target="_blank"><?php echo $row_rsVendor['plan_name']; ?></a></td>
            <td valign="top" class="tdcview2"><?php echo $row_rsVendor['plan_code']; ?></td>
            <td valign="top" class="tdcview2"><?php echo $row_rsVendor['plan_desc']; ?></td>
            <td valign="top" class="tdcview2"><a href="view_vendors_add.php?employer_id=<?php echo $_GET['employer_id']; ?>&vendor_id=<?php echo $row_rsVendor['vendor_id']; ?>&plan_id=<?php echo $row_rsVendor['plan_id']; ?>&menuTopItem=<?php echo $_REQUEST['menuTopItem']; ?>">Add Vendor</a> </td>
          </tr>
          <?php } while ($row_rsVendor = mysql_fetch_assoc($rsVendor)); ?>
        </table>
        <p> Records <?php echo ($startRow_rsVendor + 1) ?> to <?php echo min($startRow_rsVendor + $maxRows_rsVendor, $totalRows_rsVendor) ?> of <?php echo $totalRows_rsVendor ?></p>
        <table border="0" width="50%" align="center">
          <tr>
            <td width="23%" align="center"><?php if ($pageNum_rsVendor > 0) { // Show if not first page ?>
                <a href="<?php printf("%s?pageNum_rsVendor=%d%s", $currentPage, 0, $queryString_rsVendor); ?>">First</a>
                <?php } // Show if not first page ?>
            </td>
            <td width="31%" align="center"><?php if ($pageNum_rsVendor > 0) { // Show if not first page ?>
                <a href="<?php printf("%s?pageNum_rsVendor=%d%s", $currentPage, max(0, $pageNum_rsVendor - 1), $queryString_rsVendor); ?>">Previous</a>
                <?php } // Show if not first page ?>
            </td>
            <td width="23%" align="center"><?php if ($pageNum_rsVendor < $totalPages_rsVendor) { // Show if not last page ?>
                <a href="<?php printf("%s?pageNum_rsVendor=%d%s", $currentPage, min($totalPages_rsVendor, $pageNum_rsVendor + 1), $queryString_rsVendor); ?>">Next</a>
                <?php } // Show if not last page ?>
            </td>
            <td width="23%" align="center"><?php if ($pageNum_rsVendor < $totalPages_rsVendor) { // Show if not last page ?>
                <a href="<?php printf("%s?pageNum_rsVendor=%d%s", $currentPage, $totalPages_rsVendor, $queryString_rsVendor); ?>">Last</a>
                <?php } // Show if not last page ?>
            </td>
          </tr>
        </table>
        <?php } // Show if recordset not empty ?>
        <?php if ($totalRows_rsVendor == 0) { // Show if recordset empty ?>
        <p>No <?php echo DISPLAYPLANNAME; ?> in list. </p>
...
<table width="100%" border="6" cellspacing="0" cellpadding="3" class="greentbl">
    <tr valign="bottom" >
        <td colspan="2" class="greenth"><font color="#ffffff" face="Tahoma" size="2">Active Vendor <?php echo DISPLAYPLANNAME; ?>s</font></td>
    </tr>
    <tr valign="top" >
      <td colspan="2" class="greentd">
	  	<?php if ($totalRows_rsVendorActive > 0) { // Show if recordset not empty ?>
        <table width="100%" border="6" cellspacing="0" cellpadding="5" style="border-style:solid" bordercolor="#999999" class="tbl">
          <tr>
            <td valign="top" class="thcview6"><strong>Vendor</strong></td>
            <td valign="top" class="thcview6"><strong><?php echo DISPLAYPLANNAME; ?></strong></td>
            <td valign="top" class="thcview6"><strong>Contact Name </strong></td>
            <td valign="top" class="thcview6"><strong>Contact Email </strong></td>
            <td valign="top" class="thcview6"><strong>Contact Phone </strong></td>
            <td valign="top" class="thcview6"><strong>Group Plan Number </strong></td>
            <td valign="top" class="thcview6"><strong>Actions</strong></td>
          </tr>
          <?php do { ?>
          <tr>
            ...
            <td valign="top" class="tdcview2"><a href="view_vendors_make_inactive.php?vendor_id=<?php echo $row_rsVendorActive['vendor_id']; ?>&plan_id=<?php echo $row_rsVendorActive['plan_id']; ?>&employer_id=<?php echo $_GET['employer_id']; ?>&menuTopItem=<?php echo $_REQUEST['menuTopItem']; ?>">Make Inactive</a> | <a href="view_vendors_del_active.php?vendor_id=<?php echo $row_rsVendorActive['vendor_id']; ?>&plan_id=<?php echo $row_rsVendorActive['plan_id']; ?>&employer_id=<?php echo $_GET['employer_id']; ?>&menuTopItem=<?php echo $_REQUEST['menuTopItem']; ?>">Delete</a> | <a href="view_vendors_edit.php?vendor_id=<?php echo $row_rsVendorActive['vendor_id']; ?>&plan_id=<?php echo $row_rsVendorActive['plan_id']; ?>&employer_id=<?php echo $_GET['employer_id']; ?>&menuTopItem=<?php echo $_REQUEST['menuTopItem']; ?>&contact_id=<?php echo $row_rsVendorActive['contact_id']; ?>">Edit</a> 
			
				| 
				<a href="javascript:;" onclick="doAjaxLoadingText('../main/docs.php','GET','vendor_id=<?php echo $row_rsVendorActive['vendor_id']; ?>&plan_id=<?php echo $row_rsVendorActive['plan_id']; ?>&employer_id=<?php echo $_GET['employer_id']; ?>','','divDocs_<?php echo $row_rsVendorActive['vendor_id']; ?>','yes');">
					Docs
				</a>
...
<br />
<table width="100%" border="6" cellspacing="0" cellpadding="3" class="redtbl">
    <tr valign="bottom" >
        <td colspan="2" class="redth"><font color="#ffffff" face="Tahoma" size="2">Inactive Vendor <?php echo DISPLAYPLANNAME; ?>s</font></td>
    </tr>
    <tr valign="top" >
      <td colspan="2" class="redtd">
        ...
        <table width="100%" border="6" cellspacing="0" cellpadding="5" style="border-style:solid" bordercolor="#999999" class="tbl">
          ...
          <tr>
            ...
            <td valign="top" class="tdcview2"><a href="view_vendors_make_active.php?vendor_id=<?php echo $row_rsVendorDeleted['vendor_id']; ?>&plan_id=<?php echo $row_rsVendorDeleted['plan_id']; ?>&employer_id=<?php echo $_GET['employer_id']; ?>&menuTopItem=<?php echo $_REQUEST['menuTopItem']; ?>">Make Active</a> | <a href="view_vendors_del_inactive.php?vendor_id=<?php echo $row_rsVendorDeleted['vendor_id']; ?>&plan_id=<?php echo $row_rsVendorDeleted['plan_id']; ?>&employer_id=<?php echo $_GET['employer_id']; ?>&menuTopItem=<?php echo $_REQUEST['menuTopItem']; ?>">Delete</a> | <a href="view_vendors_edit.php?vendor_id=<?php echo $row_rsVendorDeleted['vendor_id']; ?>&plan_id=<?php echo $row_rsVendorDeleted['plan_id']; ?>&employer_id=<?php echo $_GET['employer_id']; ?>&menuTopItem=<?php echo $_REQUEST['menuTopItem']; ?>&contact_id=<?php echo $row_rsVendorDeleted['contact_id']; ?>">Edit</a> </td>
        </tr>
