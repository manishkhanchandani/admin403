...
if($_POST['MM_Insert']==1) {
	if($_POST['contribution_date']) {
		...
		if($_FILES) {
			...
			if($array['detailIds']) {
				mysql_query("insert into employee_contribution_transaction set transaction_date = '".addslashes(stripslashes(trim($_POST['contribution_date'])))."', employer_id = '".$_COOKIE['employer']['employer_id']."'") or die('error');
				foreach($array['detailIds'] as $i => $value) {
					...
					if($num>0) {
						...
						if($rec['employer_id']!=$_COOKIE['employer']['employer_id']) {
							$arr['wrongEmployer'][$i] = $array['detailIds'][$i][1];
							continue;
						}
...
if($arr['wrongEmployer']) {
	echo 'Following SSN does not come in your employee list.<br>';
	echo '<ul>
	';
	foreach($arr['wrongEmployer'] as $key => $value) {
		echo "<li>".$value."</li>";
	}
	echo '</ul>';
}
...
<table width="100%" border="6" cellspacing="0" cellpadding="3" class="blacktbl">
    ...
    <tr valign="top" >
      <td colspan="2" class="blacktd">
<form id="form2" name="form2" method="post" action="employee_import_contribution_confirm2.php">
...
Accept maximum and create action for refund
<br />
