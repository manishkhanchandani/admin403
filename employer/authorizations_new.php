...
if($_POST['MM_Insert']==1) {
	if($_POST['employee_selected_text']) {
		...
		if($resultDeleted) {
			...
			$sql = "update users set acting_as = NULL where login_type = 'Employee' and id in (".$resultDeletedText.")";
			$rs = mysql_query($sql) or die('error');
		}
		$resultInserted = array_diff($employee_selected, $employee_selected_array);
		if($resultInserted) {
			$resultInsertedText = implode(',',$resultInserted);
			$sql = "update users set acting_as = 'Employer' where login_type = 'Employee' and id in (".$resultInsertedText.")";
			$rs = mysql_query($sql) or die('error');
		}
	} else {			
		if($_POST['employee_id']) {
			$resultInsertedText = implode(',',$_POST['employee_id']);
			$sql = "update users set acting_as = 'Employer' where login_type = 'Employee' and id in (".$resultInsertedText.")";
			$rs = mysql_query($sql) or die('error');
		}
	}
...
mysql_select_db($database_dw_conn, $dw_conn);
$query_rsAuth = sprintf("SELECT users.acting_as, users.login_type, employee.employee_id, employee.firstname FROM employee, users  WHERE employee.employer_id = %s AND employee.employee_id = users.id", $colname_rsAuth);
...
if($totalRows_rsAuth>0) {
	do { 
		if($row_rsAuth['acting_as']=="Employer") {
