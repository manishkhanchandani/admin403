...
if (isset($_COOKIE['employer']['employer_id'])) {
  $colname_rsContribution = (get_magic_quotes_gpc()) ? $_COOKIE['employer']['employer_id'] : addslashes($_COOKIE['employer']['employer_id']);
}
mysql_select_db($database_dw_conn, $dw_conn);
$query_rsContribution = sprintf("SELECT * FROM employee_contribution, employee, vendor_plan WHERE employee_contribution.employee_id = employee.employee_id AND employee_contribution.plan_id = vendor_plan.plan_id AND employee.employer_id = %s ORDER BY employee_contribution.contribution_date DESC", $colname_rsContribution);
...
<?php inc_import_form($action='employee_import_contribution_confirmation.php', $confirmLink=HTTPPATH."/main/confirm_upload_date.php", $employer_id=$_COOKIE['employer']['employer_id'], $menuTopItem=2); ?>
<!-- <form action="employee_import_contribution_confirm.php" method="post" enctype="multipart/form-data" name="form1" id="form1">
<table border="6" cellspacing="0" cellpadding="5" style="border-style:solid" bordercolor="#999999" class="tbl">
    ...
    <tr>
      <td class="thc2">&nbsp;</td>
      <td class="tdc2">File columns must be ssn, sra_pretax, sra_roth, first column should not contain data, they should be having only field names. </td>
    </tr>
    <tr>
      <td class="thc2">Date</td>
      <td class="tdc2"><input name="contribution_date" type="text" id="contribution_date" /> 
        <span class="tdc2">
        <input type="button" value="Pick date" onclick="pickDate(this,document.form1.contribution_date);" />
        </span>Date format should be yyyy-mm-dd </td>
    </tr>
    <tr>
      <td class="thc2">&nbsp;</td>
      <td class="tdc2"><input type="button" name="Submit" value="Confirm" onclick="doAjaxLoadingText('../main/confirm_upload_date.php','GET','file='+document.form1.userfile.value+'&employer_id=<?php echo $_COOKIE['employer']['employer_id']; ?>&date='+document.form1.contribution_date.value,'','divConfirm','yes');" /><div id="divConfirm"></div>
      <input name="MM_Insert" type="hidden" id="MM_Insert" value="1" /><input name="menuTopItem" type="hidden" id="menuTopItem" value="2" /></td>
    </tr>
    <tr>
      <td class="thc2">&nbsp;</td>
      <td class="tdc2"><a href="../main/employer_contribution.xls" target="_blank">Sample File</a> | <a href="../main/employer_contribution_csv.csv">Sample CSV File</a> </td>
    </tr>
  </table>
</form> -->
</div>
