...
mysql_select_db($database_dw_conn, $dw_conn);
$query_rsEmployee = sprintf("SELECT employee.employee_id, employee.email, employee.firstname, employee.ssn, employee.employer_id, employer.roth_provision FROM employee, employer WHERE employee.employee_id = %s AND employee.employer_id = employer.employer_id", $colname_rsEmployee);
...
<form action="employee_contribution_confirmation.php" method="post" name="form1" id="form1" onsubmit="MM_validateForm('contribution_date','','R','sra_pretax','','RisNum');return document.MM_returnValue">
...
<input type="hidden" name="employee_id" value="<?php echo $_GET['employee_id']; ?>" />
  ...
  <input name="ssn" type="hidden" id="ssn" value="<?php echo $encryption->processDecrypt('ssn', $row_rsEmployee['ssn']); ?>" />
</span>
</form>
</div>
...
<table width="100%" border="6" cellspacing="0" cellpadding="5" style="border-style:solid" bordercolor="#999999" class="tbl">
      <tr>
        ...
        <td class="thcview2"><strong>Employee ID</strong></td>
      </tr>
      <?php do { ?>
      <tr>
        ...
        <td class="tdcview2"><?php echo $row_rsContribution['employee_id']; ?></td>
      </tr>
