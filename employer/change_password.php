...
if (isset($_COOKIE['employer']['employer_id'])) {
  $colname_rsEdit = (get_magic_quotes_gpc()) ? $_COOKIE['employer']['employer_id'] : addslashes($_COOKIE['employer']['employer_id']);
}
mysql_select_db($database_dw_conn, $dw_conn);
$query_rsEdit = sprintf("SELECT employer_id, password, email, name FROM employer WHERE employer_id = %s", $colname_rsEdit);
	...
	if($_POST['oldpassword'] && $_POST['newpassword'] && $_POST['confirmpassword']) {
		if(md5($_POST['oldpassword'])==$row_rsEdit['password']) {
			if($_POST['newpassword']==$_POST['confirmpassword']) {
				$sql = "update employer set password = '".md5($_POST['newpassword'])."' where employer_id = '".$_COOKIE['employer']['employer_id']."'";
      ...
      <td colspan="2" class="blacktd">
	  	...
	  	<table border="6" cellspacing="0" cellpadding="5" style="border-style:solid" bordercolor="#999999" class="tbl">
			...
			<tr valign="baseline">
			  ...
			  <td class="tdc2"><?php echo $row_rsEdit['name']; ?></td>
		  </tr>
			<tr valign="baseline">
