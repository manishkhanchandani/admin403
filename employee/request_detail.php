...
if (isset($_COOKIE['employee']['employee_id'])) {
  $colid_rsDetails = (get_magic_quotes_gpc()) ? $_COOKIE['employee']['employee_id'] : addslashes($_COOKIE['employee']['employee_id']);
}
$colname_rsDetails = "-1";
if (isset($_GET['id'])) {
...
$query_rsDetails = sprintf("SELECT workflow.name, workflow.description, actions.* FROM actions, workflow WHERE actions.wf_id = workflow.id AND wf_id = %s AND  actions.requestor_type = 'Employee' AND  actions.requestor_id = %s", $colname_rsDetails,$colid_rsDetails);
