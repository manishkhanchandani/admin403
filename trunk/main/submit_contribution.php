...
function checkForm(i) {
	...
	doAjaxLoadingTextReturn_t1('<?php echo HTTPPATH; ?>/main/processEmployee.php','POST','','transaction_id='+document.getElementById("transaction_id").value+'&h='+document.getElementById("hire_date_"+i).value+'&d='+document.getElementById("dob_"+i).value+'&er='+document.getElementById("employer_id_"+i).value+'&pl='+document.getElementById("plan_id_"+i).value+'&sp='+document.getElementById("sra_pretax_"+i).value+'&sr='+document.getElementById("sra_roth_"+i).value+'&cd='+document.getElementById("contribution_date_"+i).value+'&f='+document.getElementById("firstname_"+i).value+'&l='+document.getElementById("lastname_"+i).value+'&m='+document.getElementById("middlename_"+i).value+'&a='+document.getElementById("account_number_"+i).value+'&s='+document.getElementById("ssn_"+i).value+'&employer_id='+document.getElementById("employer_id").value,'showdiv_'+i,'yes','divtr_'+i,document.getElementById("transaction_id").value);
...
if($_POST['MM_Insert']==1) {
	if($_POST['contribution_date']) {
		...
		if($_FILES) {
			...
			$sess = postProcessMod($array, $_POST);
		}
	} else {
...
if($_POST['transaction_id']) {	
	...
	updateTransactionTable($_POST['contribution_date'], $_POST['employer_id']);
	//updateTransaction($sess, $_POST, $transactionId);
	?>
	<div id="transactionTable"><?php echo showTransactionTable($transactionId); ?></div>	
	<?php
} else {
...
?>
<?php
if($sess['post']['vendorinactivessn']) {
	$string = implode(', ', $sess['post']['vendorinactivessn']);
	echo '<p><b>Vendor Inactive for SSN:</b> '. $string.'</p>';
}
if($sess['post']['contributionalreadyaddedforthisdate']) {
	$string = implode(', ', $sess['post']['contributionalreadyaddedforthisdate']);
	echo '<p><b>Contribution already added for SSN:</b> '. $string.'</p>';
}
...
<table width="100%" border="6" cellspacing="0" cellpadding="3" class="blacktbl">
    ...
    <tr valign="top" >
      <td colspan="2" class="blacktd">
			<table width="100%" border="6" cellspacing="0" cellpadding="0" style="border-style:solid" bordercolor="#999999" class="tbl">
			<tr>
		...
		<input type="hidden" name="transaction_id" id="transaction_id" value="<?php echo $transactionId; ?>" /><input type="hidden" name="employer_id" id="employer_id" value="<?php echo $_POST['employer_id']; ?>" /><input type="hidden" name="total" value="<?php echo $i; ?>" />
