...
if($_POST['MM_Insert']==2) {
	...
	if($resultDeleted) {
		...
		$sql = "update employer_vendor set active = 0 where employer_id = '".$_COOKIE['employer']['employer_id']."' and plan_id in (".$resultDeletedText.")";
	...
	if($resultInserted) {
		...
		$sql = "update employer_vendor set active = 1 where employer_id = '".$_COOKIE['employer']['employer_id']."' and plan_id in (".$resultInsertedString.")";
		$rs = mysql_query($sql) or die('error');
	}
}
if($_POST['MM_Insert']==1) {
	...
	if($_POST['vendor_selected_text']) {
		...
		if($resultDeleted) {
			$resultDeletedText = implode(',',$resultDeleted);
			$sql = "delete from employer_vendor where employer_id = '".$_COOKIE['employer']['employer_id']."' and plan_id in (".$resultDeletedText.")";
			$rs = mysql_query($sql) or die('error');
		}
		...
		if($resultInserted) {
			...
			foreach($resultInserted as $value) {
				if($value!="") {
					$sql2[] = "('".$_COOKIE['employer']['employer_id']."', '".$value."', '".$return[$value]."')";
				}
			}
			$sql3 = implode(', ', $sql2);
			$sql .= $sql3;
	...
	header("Location: manage_vendors.php?menuTopItem=3");
	exit;
}
?>
<?php
...
if (isset($_COOKIE['employer']['employer_id'])) {
  $colname_rsEmployer = (get_magic_quotes_gpc()) ? $_COOKIE['employer']['employer_id'] : addslashes($_COOKIE['employer']['employer_id']);
}
...
if (isset($_COOKIE['employer']['employer_id'])) {
  $colname_rsVendorEmployer = (get_magic_quotes_gpc()) ? $_COOKIE['employer']['employer_id'] : addslashes($_COOKIE['employer']['employer_id']);
}
mysql_select_db($database_dw_conn, $dw_conn);
...
<table width="100%" border="6" cellspacing="0" cellpadding="5" style="border-style:solid" bordercolor="#999999" class="tbl">
  <tr>
    ...
    <td valign="top" class="thcview2"><strong>Employer Account Access URL</strong></td>
...
?>
  <tr>
    ...
    <td valign="top" class="tdcview2"><?php if($vends[$value['vendor_id']]['employer_access']) { ?><a href="<?php echo $vends[$value['vendor_id']]['employer_access'];  ?>" target="_blank">Click Here</a><?php } ?>&nbsp;</td>
