...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "form1")) {
	if($_POST['email']) {
		...
		if(mysql_num_rows($rs)>0) {
			...
			if($rec['id']==$_POST['employee_id']) {
			
			} else {
				$error .= "Email already exists. Please choose any other email. ";
				unset($_POST["MM_update"]);
			}
		} else {
			if($_POST['password'] && $_POST['email']) {
				$sql = "INSERT INTO `users` (`email` , `password` , `created_dt` , `login_type` , `acting_as`, `id` ) VALUES ('".addslashes(stripslashes(trim($_POST['email'])))."', '".md5($_POST['password'])."', '".date('Y-m-d H:i:s')."', 'Employee', NULL, '".$_POST['employee_id']."')";
		...
		if($_POST["MM_update"] == "form1") {
			if($_POST['password']) {
				mysql_query("UPDATE employee set email = '".addslashes(stripslashes(trim($_POST['email'])))."', password = '".md5($_POST['password'])."' where employee_id = '".$_POST['employee_id']."'") or die('error');
			} else if($_POST['email']) {
				mysql_query("UPDATE employee set email = '".addslashes(stripslashes(trim($_POST['email'])))."' where employee_id = '".$_POST['employee_id']."'") or die('error');
			}
		}
	}
}
...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "form1")) {
  ...
  $updateGoTo = "manage_employees.php";
  if (isset($_SERVER['QUERY_STRING'])) {
...
if (isset($_GET['employee_id'])) {
  $colname_rsEdit = (get_magic_quotes_gpc()) ? $_GET['employee_id'] : addslashes($_GET['employee_id']);
}
mysql_select_db($database_dw_conn, $dw_conn);
$query_rsEdit = sprintf("SELECT * FROM employee WHERE employee_id = %s", $colname_rsEdit);
...
<form action="<?php echo $editFormAction; ?>" method="POST" name="form1">
      <table border="6" cellspacing="0" cellpadding="5" style="border-style:solid" bordercolor="#999999" class="tbl">
        <tr valign="baseline">
          <td align="right" nowrap="nowrap" class="thc2">Email:</td>
          <td class="tdc2"><input type="text" name="email" value="<?php echo $row_rsEdit['email']; ?>" id="email" size="50" /></td>
        </tr>
        <tr valign="baseline">
          <td align="right" nowrap="nowrap" class="thc2">Password:</td>
          <td class="tdc2"><input name="password" type="password" id="password" /></td>
        </tr>
        <tr valign="baseline">
          <td align="right" nowrap="nowrap" class="thc2">SSN:</td>
          <td class="tdc2"><?php echo $encryption->processDecrypt('ssn', $row_rsEdit['ssn']); ?></td>
        </tr>
        <tr valign="baseline">
...
?>
        ...
        <tr valign="baseline">
          ...
          <td class="tdc2"><input type="text" name="hire_date" value="<?php echo $row_rsEdit['hire_date']; ?>" size="32">
        ...
        <tr valign="baseline">
          ...
          <td class="tdc2"><input type="text" name="termination_date" value="<?php echo $row_rsEdit['termination_date']; ?>" size="32">
            <input type="button" value="Pick date" onclick="pickDate(this,document.form1.termination_date);" /></td>
        </tr>
        <tr valign="baseline">
          <td align="right" nowrap class="thc2">Dob:</td>
          <td class="tdc2"><input type="text" name="dob" value="<?php echo $encryption->processDecrypt('dob', $row_rsEdit['dob']); ?>" size="32">
            <input type="button" value="Pick date" onclick="pickDate(this,document.form1.dob);" /></td>
        </tr>
        <tr valign="baseline">
          <td align="right" nowrap class="thc2">Account_number:</td>
          <td class="tdc2"><input type="text" name="account_number" value="<?php echo $encryption->processDecrypt('account_number', $row_rsEdit['account_number']); ?>" size="32"></td>
        </tr>
        <tr valign="baseline">
