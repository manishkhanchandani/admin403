...
$query_rsPlan = sprintf("SELECT p.plan_name, p.plan_id  FROM employee_vendor as ev, vendor_plan as p  WHERE ev.plan_id = p.plan_id and ev.employee_id = %s", $colname_rsPlan);
	...
	*/
    ...
    Detailed Transactions </td>
			...
			<tr>
			  ...
			  <td valign="top" class="tdcview2"><table  border="0" cellspacing="0" cellpadding="5">
                <tr>
...
do {  
?>
                    <option value="<?php echo $row_rsPlan['plan_id']?>"><?php echo $row_rsPlan['plan_name']?></option>
                    <?php
} while ($row_rsPlan = mysql_fetch_assoc($rsPlan));
  $rows = mysql_num_rows($rsPlan);
  if($rows > 0) {
      mysql_data_seek($rsPlan, 0);
	  $row_rsPlan = mysql_fetch_assoc($rsPlan);
  }
?>
                                    </select></td>
