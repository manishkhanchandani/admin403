...
if (isset($_COOKIE['vendor_id'])) {
  $colname_rsEmployer = (get_magic_quotes_gpc()) ? $_COOKIE['vendor_id'] : addslashes($_COOKIE['vendor_id']);
}
mysql_select_db($database_dw_conn, $dw_conn);
$query_rsEmployer = sprintf("SELECT e.employer_id, e.name FROM employer_vendor as ev, vendor_plan as p, employer as e  WHERE ev.plan_id = p.plan_id AND p.vendor_id = %s AND ev.employer_id = e.employer_id  GROUP BY ev.employer_id", $colname_rsEmployer);
...
<title>Vendor :: Remittance by Employer report</title>
<!-- InstanceEndEditable -->
<?php include('css.php'); ?>
<?php include('js.php'); ?>
...
<script type="text/javascript">
	...
	*/
	...
	<div id="calendarForForm">
	  <form name="searchForm" method="get" action="report_remittance_by_employer_result.php">
      
	    <table width="100%" border="6" cellspacing="0" cellpadding="3" class="greentbl">
          <tr>
            <td class="greenth"><font color="#ffffff" face="Tahoma" size="2">Remittance by Employer Report</font></td>
          </tr>
          <tr>
            <td colspan="3" align="center" valign="top" class="greentd"><table border="0" cellspacing="0" cellpadding="5">
              <tr>
                <td>Employer:</td>
...
?>
          ...
          <tr>
            <td align="center" valign="top" class="greentd"><input type="submit" name="VendorSubmit" id="VendorSubmit" value="Get Total Remittance" />
