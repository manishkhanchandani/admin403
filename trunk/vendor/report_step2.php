...
if($_GET['report_view']==1) {
	if($_GET['report_type']==1) {
		...
		$query = "select sum(ec.sra_pretax) as pretax, sum(ec.sra_roth) as roth, vp.plan_name, ec.employee_id, ec.vendor_id, ec.employer_id, ec.contribution_date from employee_contribution as ec, vendor_plan as vp Where ec.vendor_id = '".$vendor_id."' and ec.plan_id like '".$plan_id."' and ec.employer_id like '".$employer_id."' and ec.plan_id = vp.plan_id";
		if($_GET['fromDate']) {
			$f = $_GET['fromDate'];
		} else {
			$f = date('Y')."-01-01";
		...
		$query .= " AND ec.contribution_date between '".$f."' and '".$t."' GROUP BY vp.plan_id ORDER BY ec.plan_id, ec.employer_id";
		...
		$query = "select ec.sra_pretax as pretax, ec.sra_roth as roth, vp.plan_name, ec.employee_id, ec.vendor_id, ec.employer_id, ec.contribution_date from employee_contribution as ec, vendor_plan as vp Where ec.vendor_id = '".$vendor_id."' and ec.plan_id like '".$plan_id."' and ec.employee_id like '".$employee_id."' and ec.plan_id = vp.plan_id";
		if($_GET['fromDate']) {
			$f = $_GET['fromDate'];
		} else {
			$f = date('Y')."-01-01";
		...
		$query .= " AND ec.contribution_date between '".$f."' and '".$t."' GROUP BY vp.plan_id ORDER BY ec.plan_id, ec.employee_id";
		$rsPlan = mysql_query($query) or die('error in plan'.mysql_error());
		if(mysql_num_rows($rsPlan)>0) {
			while($rowPlan = mysql_fetch_array($rsPlan)) {
				$return[] = $rowPlan;
	...
	if($_GET['report_type']==1) {
		...
		$query = "select ec.sra_pretax as pretax, ec.sra_roth as roth, vp.plan_name, ec.employee_id, ec.vendor_id, ec.employer_id, ec.contribution_date from employee_contribution as ec, vendor_plan as vp Where ec.vendor_id = '".$vendor_id."' and ec.plan_id like '".$plan_id."' and ec.employer_id like '".$employer_id."' and ec.plan_id = vp.plan_id";
		if($_GET['fromDate']) {
			$f = $_GET['fromDate'];
		} else {
			$f = date('Y')."-01-01";
		...
		$query .= " AND ec.contribution_date between '".$f."' and '".$t."' ORDER BY ec.plan_id, ec.employer_id";
		$rsPlan = mysql_query($query) or die('error in plan'.mysql_error());
		if(mysql_num_rows($rsPlan)>0) {
			while($rowPlan = mysql_fetch_array($rsPlan)) {
				$return[] = $rowPlan;
		...
		$query .= " AND ec.contribution_date between '".$f."' and '".$t."' ORDER BY ec.plan_id, ec.employee_id";
		$rsPlan = mysql_query($query) or die('error in plan'.mysql_error());
		if(mysql_num_rows($rsPlan)>0) {
			while($rowPlan = mysql_fetch_array($rsPlan)) {
				$return[] = $rowPlan;
...
<table width="100%" border="6" cellspacing="0" cellpadding="3" class="blacktbl">
    <tr valign="bottom" >
        <td colspan="2" class="blackth"><font color="#ffffff" face="Tahoma" size="2">Search Results</font></td>
    </tr>
    <tr valign="top" >
