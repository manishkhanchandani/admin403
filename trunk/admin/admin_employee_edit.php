	...
	if($_POST['email']) {
		...
		if(mysql_num_rows($rs)>0) {
			...
			if($_POST['password'] && $_POST['email']) {
				$sql = "select user_id, id from `users` where id = '".$_POST['employee_id']."' and login_type = 'Employee'";
				$rs = mysql_query($sql) or die('error in selecting .'.mysql_error());
				if(mysql_num_rows($rs)==0) {
					...
					$rec = mysql_fetch_array($rs);
					$sql = "update `users` set `email` = '".addslashes(stripslashes(trim($_POST['email'])))."', `password` = '".md5($_POST['password'])."' WHERE user_id = '".$rec['user_id']."'";
					$rs = mysql_query($sql) or die('could not insert in users table');
					$user_id = $rec['user_id'];
				}
			} else {
...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "form1")) {
  ...
  $updateGoTo = "admin_employee_list.php";
  if (isset($_SERVER['QUERY_STRING'])) {
...
if (isset($_GET['employee_id'])) {
  $colname_rsEdit = $_GET['employee_id'];
}
mysql_select_db($database_dw_conn, $dw_conn);
...
?>
        ...
        <tr valign="baseline">
          ...
          <td class="tdc2"><input type="text" name="dob" value="<?php echo $row_rsEdit['dob']; ?>" size="32">
